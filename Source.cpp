 ...
 */
int main(void) {
	ifstream inputFile_allocation;
	inputFile_allocation.open("Resources.txt");

	while (!inputFile_allocation.eof())
	{
		for (int i = 0; i < RESOURCES; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				inputFile_allocation >> allocation[i][j];
			}
		}
	}
	inputFile_allocation.close();

	ifstream inputFile_Maximum;
	inputFile_Maximum.open("Maximum.txt");

	while (!inputFile_Maximum.eof())
	{
		for (int i = 0; i < RESOURCES; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				inputFile_Maximum >> maximum[i][j];
			}
		}
	}
	inputFile_Maximum.close();
